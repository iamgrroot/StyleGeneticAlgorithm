...
content_im=cv2.imread('f2.jpg')
imsh=content_im.shape
"""
...
def initial_population(content_img,img_shape, n_individuals=6):
    """
    ...
    init_population[0, :] = adjustingcontrast.adjustCONTRAST100(content_img)
    init_population[1,:] = adjustingcontrast.adjustCONTRAST3(content_img)
    init_population[2,:]=  sharpenanddenoised.sharpen10(content_img)
    init_population[3,:]= sharpenanddenoised.sharpen3(content_img)
    init_population[4,:]= sharpenanddenoised.denoised(content_img)
    init_population[5,:]= numpy.reshape(a=content_img,newshape=(functools.reduce(operator.mul, content_img.shape)))
    return init_population

def chromosome2img(chromosome, img_shape=imsh):
    """
...
def fitness_fun(t, p):
    ...
    px=p[:,:,0]
    tx=t[:,:,0]
    pt=numpy.transpose(px)
    tt=numpy.transpose(tx)
    gp=px.dot(pt)
    gt=tx.dot(tt)
    quality = numpy.mean(numpy.abs(gt-gp))
    ...
    quality = 100000000000000000 - quality
    return quality
...
def cal_pop_fitness(target_chrom, pop):
    ...
    for indv_num in range(pop.shape[0]):
        ...
        p= chromosome2img(pop[indv_num,:])
        t= chromosome2img(target_chrom)
        qualities[indv_num] = fitness_fun(t,p)
    return qualities
...
def crossover(parents, img_shape, n_individuals=6):
    """
