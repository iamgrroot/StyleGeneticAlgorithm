...
#content_img= skimage.io.imread(fname='DSC_5123.JPG')
content_img=cv2.imread('f2.jpg')
# Reading target image to be reproduced using Genetic Algorithm (GA).
#target_img = skimage.io.imread(fname='fruit.jpg')
target_img=cv2.imread('fruit.jpg')

target_im = cv2.resize(target_img, content_img.shape[0:2], interpolation=cv2.INTER_CUBIC)
# Target image after enconding. Value encoding is used.
target_chromosome = GARI.img2chromosome(target_im)

content_chromosome= GARI.img2chromosome(content_img)
# Population size
sol_per_pop = 8
# Mating pool size
...
new_population = GARI.initial_population(content_img ,img_shape=content_img.shape, 
...
for iteration in range(20):
    ...
    GARI.save_images(iteration, qualities, new_population, target_im.shape, 
                     save_point=10, save_dir=os.curdir+'//')
